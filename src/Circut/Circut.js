...
const Circut = ({scrollToIds, onScrollTo}) =>{
    ...
    : {width:cpuSize*0.9, height: cpuSize*0.8, x: size[0]/5, y: size[1]/8, id:"chip1", innerPer:{x:0.75, y:0.75}, expandPer:{x:1.3, y:0.8}});
    ...
    : {width: cpuSize*1.6, height: cpuSize * 0.5, x:size[0]/6*4, y: size[1]/4, id:"chip2", innerPer:{x:0.7, y:0.73}, expandPer:{x:0.8, y:1.7}});
...
:{width: cpuSize*0.8, height: cpuSize * 0.5, x: size[0]/5, y: size[1]/11*7.5, id: "chip4", innerPer:{x:0.75, y:0.7}, expandPer:{x:0.8, y:2.2}});
    ...
    let cable1Path = (windowSize[0] > windowSize[1] ? [cpuProng1, {x:cpuProng1.x, y:size[1]/7}, {x:(chip1.x + cpuLoc.x)/2, y:size[1]/7}, {x:(chip1.x + cpuLoc.x)/2, y:size[1]/2.6}, {x:chip1.x + iowidth*2.4,  y:size[1]/2.6 }, {x:chip1.x + iowidth*2.4, y:size[1]/4 + chip1.height/2}]
    :[cpuProng1, {x:cpuProng1.x, y:size[1]/4}, {x:chip1.x + chip1.width/5, y:size[1]/4}, {x:chip1.x + chip1.width/5, y:chip1.y}]);
    let cable2Path = (windowSize[0] - windowSize[1] > -150 ? [cpuProng2, {x:(cpuProng2.x+chip2.x-chip2.width/2)/2, y:cpuProng2.y}, {x:(cpuProng2.x+chip2.x-chip2.width/2)/2, y:chip2.y - iowidth *3}, {x:chip2.x-chip2.width/2, y:chip2.y - iowidth *3}]
    : [cpuProng2, {x:chip2.x + chip2.width/5 * 1.1, y:cpuProng2.y}, {x:chip2.x + chip2.width/5 * 1.1, y:chip2.y}] );
    let cable3Path =  (windowSize[0] > windowSize[1] ? [cpuProng3, {x:cpuProng3.x, y:cpuProng3.y+iowidth}, {x:chip3.x-chip3.width/3, y:cpuProng3.y+iowidth}, {x:chip3.x-chip3.width/3, y:chip3.y-chip3.height/2-iowidth}, {x:chip3.x + iowidth*3.3, y:chip3.y-chip3.height/2-iowidth}, {x:chip3.x + iowidth*3.3, y:chip3.y}]
    : [cpuProng3,  {x:cpuProng3.x, y:chip3.y-chip3.height/2-iowidth}, {x:chip3.x + iowidth*3.3, y:chip3.y-chip3.height/2-iowidth}, {x:chip3.x + iowidth*3.3, y:chip3.y}] );
  ...
  : [cpuProng4, {x: cpuProng4.x-iowidth*2, y:cpuProng4.y}, {x:cpuProng4.x-iowidth*2, y: chip4.y}] );
    
  let projectText = windowSize[0] > windowSize[1] ? (<>P<br/>R<br/>O<br/>J<br/>E<br/>C<br/>T<br/>S</>) : "PROJECTS";
  let experienceText = windowSize[0] > windowSize[1] ? (<>E<br/>X<br/>P<br/>E<br/>R<br/>I<br/>E<br/>N<br/>C<br/>E</>) : "EXPERIENCE";
  ...
  anime.set(".chip-click-gradient", {translateY:"-75%"})
    ...
    return (
        ...
        <Chip title={projectText} chipId={chip4.id} width={chip4.width} height={chip4.height} x={chip4.x} y={chip4.y} innerPer={chip4.innerPer} expandPer={chip4.expandPer} iowidth={iowidth} onClick={()=>{onScrollTo(scrollToIds[1])}} hideBydDefault={true} handleTouchSeperately={true}/>
        <Chip title={experienceText} chipId={chip2.id} width={chip2.width} height={chip2.height} x={chip2.x} y={chip2.y} innerPer={chip2.innerPer} expandPer={chip2.expandPer} onClick={()=> {onScrollTo(scrollToIds[2])}}  iowidth={iowidth} hideBydDefault={true} handleTouchSeperately={true}/>
